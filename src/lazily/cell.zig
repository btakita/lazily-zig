        ...
        pub fn init(
            ...
            const self = slot(
                ...
                deinitSlotValue(Cell(T), struct {
                    fn deinitValue(_ctx: *Context, _getCell: *const ValueFn(Cell(T)), _cell: Cell(T)) void {
                        _ = _ctx;
                        _ = _getCell;
...
test "cellFn: get/set + invalidate cache" {
    ...
    const name = comptime initCellFn(String, struct {
    ...
    const getGreeting = struct {
        fn call(_ctx: *Context) !OwnedString {
            std.debug.print("getGreeting#d1\n", .{});
    ...
    const getGreetingAndResponse = struct {
        fn call(_ctx: *Context) !OwnedString {
            ...
            std.debug.print("getGreetingAndResponse#d1, geeting={s}\n", .{(try greeting(_ctx)).value});
            return OwnedString.managed(
                std.fmt.allocPrint(
                    _ctx.allocator,
                    "{s} {s}",
    ...
    try struct {
        fn call(_ctx: *Context) !void {
            ...
            std.debug.print("test#before set#d1\n", .{});
            name_cell.set("You");
            std.debug.print("test#after set#d1\n", .{});
    ...
    try std.testing.expectEqual(
        null,
        ctx.getSlot(getGreeting),
    );
    try std.testing.expectEqual(
        null,
        ctx.getSlot(getGreetingAndResponse),
    );
    try expectEventLog(ctx, "greeting|hello|name|greetingAndResponse|response|");

    std.debug.print("test#before failure#d1\n", .{});
    try std.testing.expectEqualStrings(
        "Hello You!",
        (try getGreeting(ctx)).value,
    );
    ...
    std.debug.print("test#after failure#d1\n", .{});
    ...
    try std.testing.expect(ctx.getSlot(getGreeting) != null);
    try std.testing.expect(ctx.getSlot(getGreetingAndResponse) != null);
    try expectEventLog(ctx, "greeting|hello|name|greetingAndResponse|response|greetingAndResponse|response|greeting|");
}
